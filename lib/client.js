...
/* ---- 8. Thinking summary: vertical rolling window (line by line up) ---- */
.dshut-vmarquee {
  flex: auto;
  min-width: 0;
  height: 48px;
...
/* ---- 9. Turn status: replace "Deep diving..." with an animated SVG ---- */
.dshut-status {
  flex: none;
  margin-right: 8px;
}
...
.dshut-status-dot {
  ...
  filter: drop-shadow(0 0 5px var(--dsw-static-deepseek-400));
  animation: dshut-status-pulse 1.2s ease-in-out infinite;
}
.dshut-status-dot:nth-child(2) {
  animation-delay: 0.15s;
}
.dshut-status-dot:nth-child(3) {
  animation-delay: 0.3s;
}
@keyframes dshut-status-pulse {
...
`;
		...
		// ------------------------------------------------------------------
		// Thinking summary: vertical rolling window while streaming.
		// The official UI replaces the one-line summary on every delta (which
		// reads as flicker). Instead we keep a small fixed-height window that
		// shows the last two lines; when a new line arrives (the summary text
		// no longer continues the previous one), the previous line is frozen
		// above it and the whole track rolls up one line-height smoothly,
		// like a terminal or an odometer. When the turn ends the window
		// collapses back to the single first line without animation.
		// ------------------------------------------------------------------
		function initThinkingScroller() {
			if (typeof document === "undefined" || document.body === null) return;
			const LINE_H = 24;
			const MAX_ROWS = 10;
			...
			const wrap = (summary) => {
				...
				if (summary.closest(".dshut-vmarquee") !== null) return;
				const parent = summary.parentNode;
				if (parent === null) return;
				const win = el("div", "dshut-vmarquee");
				const track = el("div", "dshut-vmarquee-track");
				summary.classList.add("dshut-vline");
				parent.insertBefore(win, summary);
				track.append(summary);
				win.append(track);

				let offset = 0;
				let lastText = summary.textContent;

				// Freeze the finished line above the live one and roll up one row.
				const pushLine = (text) => {
					const line = el("div", "dshut-vline", text);
					track.insertBefore(line, summary);
					offset += LINE_H;
					track.style.transform = "translateY(-" + offset + "px)";
					while (track.children.length > MAX_ROWS) {
						const first = track.firstElementChild;
						if (first === null || first === summary) break;
						first.remove();
					}
				};
				...
				const sync = () => {
					...
					// A new line arrived when the new text no longer continues
					// the previous one (append-only while streaming).
					if (lastText !== "" && text !== lastText && !text.startsWith(lastText) && !lastText.startsWith(text)) {
						pushLine(lastText);
					}
					lastText = text;
				};
				...
				textObserver.observe(summary, { childList: true, characterData: true, subtree: true });

				// When the turn ends, collapse to a single static line (the
				// official first-line summary) without any rolling animation.
				const root = summary.closest('[data-variant="think"]');
				const applyState = () => {
					const running = root !== null && root.dataset.state === "running";
					if (running) return;
					while (track.firstElementChild !== null && track.firstElementChild !== summary) {
			...
			const observer = new MutationObserver((mutations) => {
				for (const mutation of mutations) {
					for (const node of mutation.addedNodes) {
						if (!(node instanceof Element)) continue;
						if (node.matches('[data-variant="think"] [data-follow-end]')) wrap(node);
						const found = node.querySelectorAll('[data-variant="think"] [data-follow-end]');
						for (const summary of found) wrap(summary);
					}
				}
			});
			observer.observe(document.body, { childList: true, subtree: true });
			for (const summary of document.querySelectorAll('[data-variant="think"] [data-follow-end]')) wrap(summary);
		}

		// ------------------------------------------------------------------
		// Turn status: replace the "Deep diving..." shimmer text with an
		// animated SVG (three glowing pulses). The official component renders
		// the label as a bare text node inside div[class$="_turnStatus"] and a
		// clock span after ~15s; we wrap the text node in a hidden span and
		// prepend the SVG. React updates the text node in place (its fiber
		// reference survives), so the hiding never leaks.
		// ------------------------------------------------------------------
		function initTurnStatusAnimation() {
			if (typeof document === "undefined" || document.body === null) return;
			const SVG_NS = "http://www.w3.org/2000/svg";

			const wrap = (status) => {
				if (!(status instanceof Element)) return;
				if (status.querySelector(".dshut-status") !== null) return;
				// Hide bare text children (the label) but keep the clock span.
				for (const child of [...status.childNodes]) {
					if (child.nodeType === Node.TEXT_NODE && child.textContent.trim() !== "") {
						const hidden = el("span", "dshut-status-hidden");
						status.insertBefore(hidden, child);
						hidden.appendChild(child);
					}
				}
				// Inject the animated SVG.
				const svg = document.createElementNS(SVG_NS, "svg");
				svg.setAttribute("class", "dshut-status");
				svg.setAttribute("width", "52");
				svg.setAttribute("height", "26");
				svg.setAttribute("viewBox", "0 0 52 26");
				svg.setAttribute("aria-hidden", "true");
				for (const cx of [12, 26, 40]) {
					const circle = document.createElementNS(SVG_NS, "circle");
					circle.setAttribute("class", "dshut-status-dot");
					circle.setAttribute("cx", String(cx));
					circle.setAttribute("cy", "13");
					circle.setAttribute("r", "4.5");
					svg.appendChild(circle);
				}
				status.insertBefore(svg, status.firstChild);
			};
			...
			const observer = new MutationObserver((mutations) => {
				for (const mutation of mutations) {
					for (const node of mutation.addedNodes) {
						if (!(node instanceof Element)) continue;
						if (node.matches('[class$="_turnStatus"]')) wrap(node);
						const found = node.querySelectorAll('[class$="_turnStatus"]');
						for (const status of found) wrap(status);
					}
				}
			});
			observer.observe(document.body, { childList: true, subtree: true });
			for (const status of document.querySelectorAll('[class$="_turnStatus"]')) wrap(status);
		}
		...
		*/
		...
		function apply(ctx) {
			...
			const mount = () => {
				...
				initComposerHandle();
				initThinkingScroller();
				initTurnStatusAnimation();
				applyFonts();
			};
