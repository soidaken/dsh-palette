...
/* ---- 8. Thinking summary: seamless left-to-right marquee ---- */
.dshut-marquee {
  flex: auto;
  min-width: 0;
  overflow: hidden;
...
.dshut-marquee-track {
  ...
  animation: dshut-marquee-ltr 14s linear infinite;
}
.dshut-marquee-track[data-running="false"] {
...
`;
		...
		// ------------------------------------------------------------------
		// Thinking summary: seamless left-to-right marquee while streaming.
		// The official UI replaces the one-line summary on every delta (which
		// reads as flicker); we relocate the summary into a two-copy track and
		// let a CSS animation scroll it continuously, syncing the clone on
		// text changes and pausing when the turn is no longer running.
		// ------------------------------------------------------------------
		function initThinkingMarquee() {
			if (typeof document === "undefined" || document.body === null) return;

			const wrap = (summary) => {
				if (!(summary instanceof Element)) return;
				if (summary.closest(".dshut-marquee") !== null) return;
				const parent = summary.parentNode;
				if (parent === null) return;
				const marquee = el("div", "dshut-marquee");
				const track = el("div", "dshut-marquee-track");
				const clone = el("span", "dshut-marquee-item");
				summary.classList.add("dshut-marquee-item");
				parent.insertBefore(marquee, summary);
				track.append(summary, clone);
				marquee.append(track);
				// The summary is now inside the track; sync the clone with its
				// text and scale the scroll duration to the content length.
				const sync = () => {
					clone.textContent = summary.textContent;
					const length = summary.textContent.length;
					const duration = Math.min(30, Math.max(6, length * 0.22));
